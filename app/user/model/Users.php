...
class Users extends Controller
{
    ...
    public function getItemList($cid = null, $page = 1, $perPage = 10, $orderBy = 'publish_time', $order = 'desc', $where = null,$keywords = null,$whereArray = null,$uuids = null)
    {
        ...
        if ($cid == '' || $cid == null) {
            ...
            if ($itemCategoryList) {
                $itemList = db($this->item)->where($where)->where($keywordsWhere)->where($uuidsWhere)->where($whereArray)->whereOr('group_id',$cid)->whereOr('group_id','in',$cids)->page($page,$perPage)->order($orderBy,$order)->select();
            } else {
